...
CREATE TABLE TB_POST_LIKE( --좋아요 테이블
    ...
    POST_LIKE_CREATTIME DATE DEFAULT SYSDATE, --좋아요 누른 시간
...
ALTER TABLE TB_COMMENT_LIKE ADD CONSTRAINT TB_COMMENT_LIKE_LIKE_PK PRIMARY KEY(LIKE_NO); -- 게시물 댓글 좋아요 댓글 : 댓글 SEQ 기본키
...
ALTER TABLE TB_POST_LIKE ADD CONSTRAINT TB_POST_LIKE_PK PRIMARY KEY(LIKE_NO); -- 게시물(TB_POST_LIKE) 테이블의 LIKE 컬럼에 기본키(PRIMARY KEY) 제약조건 추가(부여)
...
-- CREATE SEQUENCE SEQ_MEMBER;  -- TB_MEMBER의 MEMBER_NO 컬럼 SEQ (멤버 번호)
-- CREATE SEQUENCE SEQ_STUDYGROUP; -- TB_STUDYGROUP의 GROUP_NO 컬럼 SEQ (스터디그룹 번호)
-- CREATE SEQUENCE SEQ_SCHEDULE; -- TB_SCHEDULE SCHEDULE_NO 컬럼 SEQ (스케줄 번호)
-- CREATE SEQUENCE SEQ_POST; -- TB_POST의 POST_NO 컬럼 SEQ (게시글 번호)


-- CREATE SEQUENCE SEQ_PER_REV; -- TB_PER_REV의 PER_REV_NO 컬럼 SEQ(개인평가 번호)
-- CREATE SEQUENCE SEQ_GROUP_REV;


-- CREATE SEQUENCE SEQ_POST_LIKE; -- TB_POST_LIKE의 LIKE_NO 컬럼 SEQ (게시물 좋아요 번호(갯수))
